...
DIR=chroot
PREINSTALL_PKGS=openssh-server,curl,tar,gcc,libc6-dev,time,strace,sudo,less,psmisc,selinux-utils,policycoreutils,checkpolicy,selinux-policy-default,debian-ports-archive-keyring,ethtool\
,tcpdump,libdw-dev,vim,bpftool,ncat

# If ADD_PACKAGE is not defined as an external environment variable, use our default packages
if [ -z ${ADD_PACKAGE+x} ]; then
    ADD_PACKAGE="make,sysbench,git,vim,tmux,usbutils,tcpdump,bpftool"
fi
...
if [ $PERF = "true" ]; then
    ...
    sudo chroot $DIR /bin/bash -c "apt-get update; apt-get install -y flex bison python-dev-is-python3 libelf-dev libunwind-dev libaudit-dev libslang2-dev libperl-dev binutils-dev liblzma-dev libnuma-dev make systemtap-sdt-dev python3-setuptools libpfm4-dev libcap-dev libbabeltrace-ctf-dev libssl-dev libzstd-dev libtraceevent-dev"
