// https://syzhub.intra.linuxtesting.ru/bug?id=f7b8a5e5238d290e51c462a9ed9b062d9bd3b048
...
static void initialize_tun(void)
{
	...
	write_file(sysctl, "0");
	sprintf(sysctl, "/proc/sys/net/ipv6/conf/%s/router_solicitations",
		TUN_IFACE);
	write_file(sysctl, "0");
					...
					struct join_ibss_props *ibss_props)
{
	...
	ret = nl80211_join_ibss(nlmsg, sock, nl80211_family_id, ifindex,
				ibss_props);
	if (ret < 0) {
		return -1;
	}
...
static void initialize_wifi_devices(void)
{
	...
	if (rfkill == -1) {
		...
		if (write(rfkill, &event, sizeof(event)) !=
	...
	struct join_ibss_props ibss_props = { .wiphy_freq =
						      WIFI_DEFAULT_FREQUENCY,
					      .wiphy_freq_fixed = true,
					      .mac = bssid,
					      .ssid = ssid,
					      .ssid_len = sizeof(ssid) };
	for (int device_id = 0; device_id < WIFI_INITIAL_DEVICE_COUNT;
	     device_id++) {
		mac_addr[5] = device_id;
		int ret = hwsim80211_create_device(&nlmsg, sock,
						   hwsim_family_id, mac_addr);
		if (ret < 0)
			exit(1);
		char interface[6] = "wlan0";
		interface[4] += device_id;
		if (nl80211_setup_ibss_interface(&nlmsg, sock,
						 nl80211_family_id, interface,
						 &ibss_props) < 0)
			exit(1);
	}
	for (int device_id = 0; device_id < WIFI_INITIAL_DEVICE_COUNT;
			       ...
			       volatile long a4, volatile long a5)
{
	...
	uint32_t fd_io_uring =
		syscall(sys_io_uring_setup, entries, setup_params);
...
{
	intptr_t res = 0;
	NONFAILING(syz_io_uring_setup(0x55d3, 0, 0x20ffc000, 0x20c9d000, 0, 0));
	...
	syscall(__NR_ioctl, r[1], 0x8933, 0x20000040ul);
